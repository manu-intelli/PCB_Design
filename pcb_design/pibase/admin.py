...
class PiBaseRecordAdmin(ImportExportModelAdmin):
    list_display = ('model_name', 'op_no', 'status', 'created_by', 'updated_by', 'created_at', 'updated_at')
    search_fields = ('model_name', 'op_no', 'edu_no', 'opu_no')
    list_filter = ('status', 'current_step')
    autocomplete_fields = ('model_family', 'bottom_solder_mask', 'half_moon_requirement',
                           'via_holes_on_signal_pads', 'signal_launch_type',
                           'cover_type', 'design_rule_violation_accepted', 'status',
                           'created_by', 'updated_by', 'components')
